...
public class UtilItem {
    public static LinkedList<Entry<Material, Byte>> matchItem(Player caller, String items, boolean inform) {
        ...
        if (inform && failList.length() > 0) {
            ...
            UtilPlayer.message(caller, UtilMessage.main("Item(s) Search", "" +
                    UtilChat.mBody + " Invalid [" +
                    UtilChat.mElem + failList +
                    UtilChat.mBody + "]."));
        }
    ...
    public static Entry<Material, Byte> searchItem(Player caller, String args, boolean inform) {
        ...
        if (matchList.size() != 1) {
            ...
            //Inform
            UtilPlayer.message(caller, UtilMessage.main("Item Search", "" +
                    UtilChat.mCount + matchList.size() +
                    UtilChat.mBody + " matches for [" +
                    UtilChat.mElem + args +
                    UtilChat.mBody + "]."));
            ...
            if (matchList.size() > 0) {
                ...
                for (Entry<Material, Byte> cur : matchList)
                    matchString += UtilMessage.elem(cur.getKey().toString()) + ", ";
                if (matchString.length() > 1)
                    matchString = matchString.substring(0, matchString.length() - 2);

                UtilPlayer.message(caller, UtilMessage.main("Item Search", "" +
                        UtilChat.mBody + "Matches [" +
                        UtilChat.mElem + matchString +
                        UtilChat.mBody + "]."));
            }
