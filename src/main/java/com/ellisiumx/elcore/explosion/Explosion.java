package com.ellisiumx.elcore.explosion;
...
public class Explosion implements Listener {
    public static Explosion context;
    ...
    private HashSet<FallingBlock> explosionBlocks = new HashSet<FallingBlock>();
    ...
    public void ExplosionEntity(EntityExplodeEvent event) {
        ...
        //Save
        final HashMap<Block, Map.Entry<Integer, Byte>> blocks = new HashMap<Block, Map.Entry<Integer, Byte>>();
        ...
        for (Block cur : event.blockList()) {
            ...
            if (!regenerateGround) {
                ...
                BlockRestore.Add(cur, 0, (byte) 0, (long) (20000 + (heightDiff * 3000) + Math.random() * 2900));
            }
        ...
        Bukkit.getServer().getScheduler().runTaskLater(ELCore.getContext(), new Runnable() {
            public void run() {
                //Launch
    ...
    public static void SetRegenerate(boolean regenerate) {
        context.regenerateGround = regenerate;
    }

    public static void SetDebris(boolean value) {
        context.enableDebris = value;
    }

    public static void SetLiquidDamage(boolean value) {
        context.liquidDamage = value;
    }

    public static void SetTNTSpread(boolean value) {
        context.tntSpread = value;
    }

    public static void SetTemporaryDebris(boolean value) {
        context.temporaryDebris = value;
    }

    public static HashSet<FallingBlock> GetExplosionBlocks() {
        return context.explosionBlocks;
    }

    public static void BlockExplosion(Collection<Block> blockSet, Location mid, boolean onlyAbove) {
        BlockExplosion(blockSet, mid, onlyAbove, true);
    }

    public static void BlockExplosion(Collection<Block> blockSet, Location mid, boolean onlyAbove, boolean removeBlock) {
        if (blockSet.isEmpty())
            return;
        ...
        Bukkit.getServer().getScheduler().runTaskLater(ELCore.getContext(), new Runnable() {
            public void run() {
                ...
                for (Block cur : blocks.keySet()) {
                    ...
                    double chance = 0.2 + (double) context.explosionBlocks.size() / (double) 80;
                    if (Math.random() > Math.min(0.98, chance)) {
                        ...
                        context.explosionBlocks.add(fall);
                    }
