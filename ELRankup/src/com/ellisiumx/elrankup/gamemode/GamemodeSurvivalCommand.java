...
public class GamemodeSurvivalCommand extends CommandBase {
    public GamemodeSurvivalCommand(JavaPlugin plugin) {
        super(plugin, Rank.ADMIN, "gms");
    ...
    public void execute(Player caller, String[] args) {
        if(args == null || args.length == 0) {
            ...
            caller.sendMessage(UtilMessage.main("GameMode", UtilChat.cGreen + "Set to " + GameMode.SURVIVAL.toString()));
        } else if(args[0] != null && !args[0].isEmpty()) {
            Player player = UtilPlayer.searchExact(args[0]);
            if(player != null) {
                player.setGameMode(GameMode.SURVIVAL);
                caller.sendMessage(UtilMessage.main("GameMode", UtilChat.cGreen +  args[0] + "'s Set to " + GameMode.SURVIVAL.toString()));
            } else {
