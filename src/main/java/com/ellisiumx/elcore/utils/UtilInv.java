    ...
    public static void DisallowMovementOf(InventoryClickEvent event, String name, Material type, byte data, boolean inform, boolean allInventorties) {
		...
		*/
        ...
        if (event.getAction() == InventoryAction.HOTBAR_SWAP ||
            ...
            //Inform
            UtilPlayer.message(event.getWhoClicked(), UtilMessage.main("Inventory", "You cannot hotbar swap " + UtilMessage.item(name) + "."));
        ...
        //Other
            ...
            if (!IsItem(event.getCurrentItem(), name, type, data))
                return;
            //Inform
            UtilPlayer.message(event.getWhoClicked(), UtilMessage.main("Inventory", "You cannot move " + UtilMessage.item(name) + "."));
            event.setCancelled(true);
        }
    }
